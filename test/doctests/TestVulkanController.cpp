...
TEST_CASE("Testing VulkanController" * doctest::skip(true)) {
  ...
  SUBCASE("Test Vulkan basic with shader") {
    ...
    CHECK(outputLoss.loss == doctest::Approx(params.learning_rate));
    ...
    vkDestroyDescriptorSetLayout(vulkan->logicalDevice,
  ...
  SUBCASE("Test shader test1") {
    ...
    // CHECK(result == doctest::Approx(manager.network_params.learning_rate));
    manager.network.reset();
  }
