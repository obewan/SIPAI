...
enum class CellTypeHint {
  ...
  StringWithEscapedQuotes,    ///< Quoted or unquoted string with escaped quotes
                              ///< inside.
  StringWithoutEscapedQuotes, ///< Quoted or unquoted string without any escaped
                              ///< quotes inside
...
template <std::size_t Size> class CellStringBuffer {
public:
...
private:
  ...
  [[nodiscard]] constexpr static Buffer prepareBuffer(std::string_view input,
                                                      bool has_escaped_quotes);
...
/// A value of a cell.
/// If the cell type is CellType::String, this object references the data in
/// original CSV text.
class CellReference {
public:
  /// Constructor
  ...
  getOriginalStringView(bool *has_escaped_quotes = nullptr) const;
...
/// A value of a cell.
/// This object always owns its data and does not reference the original CSV
/// text.
class CellValue {
public:
  /// Constructor
...
/// A value of a cell. All cell contents are treated as having CellType::Double
/// type. If conversion failure occurs, NaN is assumed. The data is always owned
/// by this object.
...
public:
  ...
  /// Get stored cell reference as string_view.
  /// Cell type is assumed to be CellType::String, regardless of auto-detected
  /// type. This cell may contain escaped consecutive double-quotes inside; if
  /// has_escaped_quotes is not nullptr, *has_escaped_quotes will reflect that.
  [[nodiscard]] inline constexpr std::string_view
  getOriginalStringView(bool *has_escaped_quotes = nullptr) const noexcept;

  /// Get stored cell reference as string.
  /// Cell type is assumed to be CellType::String, regardless of auto-detected
  /// type. The string has collapsed consecutive double-quotes inside.
  ...
  [[nodiscard]] constexpr CellStringBuffer<BufferSize>
  getCleanStringBuffer() const;

  /// Get required buffer size to use as getCleanStringBuffer()'s template
  /// argument. This function is useful in constexpr context.
...
/// Parser::parse*() functions use this to create the value_type object of a
/// container. By default, it handles Cell* classes and primitive numeric types.
/// This trait can be specialized for user-defined types.
template <typename CellT> class CellTrait {
public:
  /// Create an object of type CellT from cell contents represented as
  /// string_view.
  [[nodiscard]] static constexpr CellT create(std::string_view cell,
                                              CellTypeHint hint);
};
...
CellReference::CellReference(std::string_view cell, CellTypeHint hint) {
  ...
  case CellTypeHint::UnquotedData:
    if (auto double_value = readNumber<double>(cell);
...
CellReference::getOriginalStringView(bool *has_escaped_quotes) const {
  if (std::holds_alternative<String>(value_)) {
...
double CellDoubleValue::getValue() const { return value_; }
...
template <std::size_t BufferSize>
constexpr CellStringBuffer<BufferSize>
CellStringReference::getCleanStringBuffer() const {
...
const std::string &CellStringValue::getString() const { return value_; }

template <typename CellT>
constexpr CellT CellTrait<CellT>::create(std::string_view cell,
                                         CellTypeHint hint) {
  // The types here are the same as in readNumber()
  if constexpr (std::is_same_v<CellT, float> || std::is_same_v<CellT, double> ||
                std::is_same_v<CellT, long double>) {
    return readNumber<CellT>(cell).value_or(
        std::numeric_limits<CellT>::quiet_NaN());
